 ...
 */
function wc_rp_force_display( $result, $product_id ) {
	$related_ids = get_post_meta( $product_id, '_related_ids', true );

	return empty( $related_ids ) ? $result : true;
}

add_filter( 'woocommerce_product_related_posts_force_display', 'wc_rp_force_display', 10, 2 );
 ...
 */
function wc_rp_taxonomy_relation( $result, $product_id ) {
	$related_ids = get_post_meta( $product_id, '_related_ids', true );
...
add_filter( 'woocommerce_product_related_posts_relate_by_category', 'wc_rp_taxonomy_relation', 10, 2 );
add_filter( 'woocommerce_product_related_posts_relate_by_tag', 'wc_rp_taxonomy_relation', 10, 2 );

/**
 * Add related products selector to product edit screen
 */
function wc_rp_select_related_products() {
    ...
    <div class="options_group">
		...
		<?php elseif ( $woocommerce->version >= '2.3' ) : ?>
            <p class="form-field"><label for="related_ids"><?php _e( 'Related Products', 'woocommerce' ); ?></label>
                <input type="hidden" class="wc-product-search" style="width: 50%;" id="related_ids" name="related_ids"
		...
		<?php else: ?>
            <p class="form-field"><label for="related_ids"><?php _e( 'Related Products', 'woocommerce' ); ?></label>
                <select id="related_ids" name="related_ids[]" class="ajax_chosen_select_products" multiple="multiple"
                        data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce' ); ?>">
					<?php
					foreach ( $product_ids as $product_id ) {
...
add_action( 'woocommerce_product_options_related', 'wc_rp_select_related_products' );
 ...
 */
function wc_rp_save_related_products( $post_id, $post ) {
	global $woocommerce;
...
add_action( 'woocommerce_process_product_meta', 'wc_rp_save_related_products', 10, 2 );
...
add_filter( 'woocommerce_related_products_args', 'wc_rp_filter_related_products_legacy' );
 ...
 * @return array Modified query arguments.
 */
function wc_rp_filter_related_products( $query, $product_id ) {
	$related_ids = get_post_meta( $product_id, '_related_ids', true );
...
add_filter( 'woocommerce_product_related_posts_query', 'wc_rp_filter_related_products', 20, 2 );


/**
 * Create the menu item.
 */
function wc_rp_create_menu() {
	add_submenu_page( 'woocommerce', 'Custom Related Products', 'Custom Related Products', 'manage_options', 'custom_related_products', 'wc_rp_settings_page' );
}

add_action( 'admin_menu', 'wc_rp_create_menu', 99 );

/**
 * Create the settings page.
 */
function wc_rp_settings_page() {
